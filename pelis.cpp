...
void ingresar_peliculas(datoPelicula movie, int posicion,
                        Lista_peliculas &pelicula) {      //funcion para solicitar datos de clientes creados en un struct
...
void prestar(int p, Lista_peliculas &lista) {
    ...
    for (int i = primero(lista); i < fin(lista); i = siguiente(i, lista)) {
        if (i == p && aux->dato.disponible) {
            ...
            cout << "La pelicula que busca no existe o esta prestada.\n";
        }
    }
}
